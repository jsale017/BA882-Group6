...
def create_future_test_data(train_df, horizon):
    ...
    # Populate placeholder columns with last known values or recent averages
    feature_columns = [col for col in train_df.columns if col not in ['symbol', 'trade_date', 'close', 'volume']]
    for col in feature_columns:
        future_test_data[col] = train_df.groupby('symbol')[col].transform(lambda x: x.iloc[-1])  # last known value
        future_test_data[f'{col}_7d_avg'] = train_df.groupby('symbol')[col].transform(lambda x: x.rolling(7).mean().iloc[-1])  # 7-day average
